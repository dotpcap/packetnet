...
#pragma warning restore 0169, 0649
        ...
        {
            var newChecksum = CalculateChecksum(TransportPacket.TransportChecksumOption.AttachPseudoIPHeader);
            return newChecksum;
        }
        ...
        public override string ToString(StringOutputType outputFormat)
        {
            ...
            if(outputFormat == StringOutputType.Normal || outputFormat == StringOutputType.Colored)
            {
                ...
                string flags = "{";
                if (Urg)
                    flags += "urg[0x" + System.Convert.ToString((int) UrgentPointer, 16) + "]|";
                if (Ack)
                    flags += "ack[" + AcknowledgmentNumber + " (0x" + System.Convert.ToString((long) AcknowledgmentNumber, 16) + ")]|";
                if (Psh)
                    flags += "psh|";
                if (Rst)
                    flags += "rst|";
                if (Syn)
                    flags += "syn[0x" + System.Convert.ToString((long) SequenceNumber, 16) + "," + SequenceNumber + "]|";
                flags = flags.TrimEnd('|');
                flags += "}";
            ...
            if(outputFormat == StringOutputType.Verbose || outputFormat == StringOutputType.VerboseColored)
            {
                ...
                string flags = Convert.ToString((int) AllFlags, 2).PadLeft(8, '0');
