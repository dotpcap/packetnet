    ...
    public class EthernetPacketTest
    {
        ...
        public void BinarySerialization()
        {
            ...
            while ((rawCapture = dev.GetNextPacket()) != null)
            {
                ...
                Assert.AreEqual(ethernetPacket.BytesHighPerformance.BytesLength,
                    fromFile.BytesHighPerformance.BytesLength);
                Assert.AreEqual(ethernetPacket.BytesHighPerformance.Length, fromFile.BytesHighPerformance.Length);
                Assert.AreEqual(ethernetPacket.BytesHighPerformance.NeedsCopyForActualBytes,
        ...
        public void VerifyPacket0(Packet p, RawCapture rawCapture)
        {
            ...
            Assert.AreEqual(IPAddress.Parse("82.165.240.134"), ip.SourceAddress);
            Assert.AreEqual(IPAddress.Parse("192.168.1.221"), ip.DestinationAddress);
        ...
        public void VerifyPacket1(Packet p, RawCapture rawCapture)
        {
            ...
            Assert.AreEqual(IPAddress.Parse("192.168.1.104"), ip.SourceAddress);
            Assert.AreEqual(IPAddress.Parse("86.42.196.13"), ip.DestinationAddress);
        ...
        public void VerifyPacket2(Packet p, RawCapture rawCapture)
        {
            ...
            Assert.AreEqual(IPAddress.Parse("172.210.164.56"), ip.SourceAddress);
            Assert.AreEqual(IPAddress.Parse("192.168.1.104"), ip.DestinationAddress);
            Assert.AreEqual(IpVersion.IPv4, ip.Version);
        ...
        public void VerifyPacket3(Packet p, RawCapture rawCapture)
        {
            ...
            var ip = (IpPacket) p.Extract(typeof(IpPacket));
            Assert.AreEqual(IPAddress.Parse("192.168.1.172"), ip.SourceAddress);
            Assert.AreEqual(IPAddress.Parse("66.189.0.29"), ip.DestinationAddress);
            Assert.AreEqual(IPProtocolType.UDP, ip.Protocol);
        ...
        public void VerifyPacket5(Packet p, RawCapture rawCapture)
        {
            ...
            var ip = (IpPacket) p.Extract(typeof(IpPacket));
            Assert.AreEqual(IPAddress.Parse("192.168.1.104"), ip.SourceAddress);
            Assert.AreEqual(IPAddress.Parse("85.195.52.22"), ip.DestinationAddress);
        }
    }
}